...
CREATE TRIGGER insert_description_and_name_check BEFORE INSERT ON products FOR EACH ROW
BEGIN
  IF NEW.name IS NULL AND NEW.description IS NULL
    THEN SIGNAL sqlstate '45001' set message_text = "Название или описание продукта не может быть пустым"; 
  end if;
END; //

CREATE update_description_and_name_check BEFORE UPDATE ON products FOR EACH ROW
BEGIN
  IF NEW.name IS NULL AND NEW.description IS NULL
    THEN SIGNAL sqlstate '45001' set message_text = "Название или описание продукта не может быть пустым"; 
  end if;
